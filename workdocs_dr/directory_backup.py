...
class DirectoryBackupRunner():
    def __init__(self, clients: AwsClients, organization_id: str, bucket_url: str, filter: WdFilter = None, run_style: RunStyle = None) -> None:
    ...
    def get_minder(self):
        if self.minder is None:
            self.minder = DirectoryBackupMinder(self.clients, self.organization_id, self.bucket_url)
        return self.minder
    ...
    def runall(self):
        ...
        if run_style is RunStyle.ACTIVITIES:
            ...
            abr = ActivityBackupRunner(self.clients, self.organization_id, self.bucket_url, directory, self.minder)
    ...
    def _update_event_time(self, run_style: RunStyle, run_event: RunEvent) -> None:
        if self.filter is None or ((self.filter.foldernames is None or len(self.filter.foldernames) == 0)
                                   and self.filter.userquery is None and self.filter.folderpattern is None):
            self.get_minder().update_last_event_time(run_style=run_style, run_event=run_event)
