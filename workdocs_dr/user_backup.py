...
class UserBackupRunner:
    ...
    def __init__(
        self, user: UserHelper, userkeys: UserKeyHelper, clients: AwsClients
    ) -> None:
    ...
    def backup_user_queue(self, filter: WdFilter = None):
        ...
        foldertree = ListWorkdocsFolders(
            self.clients, collect_folders=True, downstream_queue=folder_queue
        )
        record_st = RecordSyncTasks(
            self.clients,
            self.userhelper,
            self.userkeyhelper,
            task_queue=folder_queue,
            downstream_queue=action_queue,
        )
        ...
        if (
            foldertree.collect_folders
            and filter.folderpattern is None
            and not filter.foldernames
        ):
    ...
    def prune_inactive_folders(self, syncer: WorkDocs2BucketSync, active_folders: set):
        ...
        s3folderids = lister.list_s3_subfoldernames(
            self.userkeyhelper.bucket, self.userkeyhelper.bucket_userprefix()
        )
        actions = []
        for folder_id in s3folderids:
            if not folder_id in active_folders:
                logging.info(
                    f"Would like to clear out folder {folder_id} for user {self.userhelper.username}"
                )
                actions.append(
                    lambda fid=folder_id: syncer.remove_folder_from_bucket(fid)
                )
    ...
    def folderidfrompath(self, path):
        ...
        for folder in subfolderlist:
            response = client.describe_folder_contents(FolderId=currentfolderid)
            matchingfolder = next(
